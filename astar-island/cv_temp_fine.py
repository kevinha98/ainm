"""Fine-grained temperature sweep around T=1.2 optimum."""
...
print("=== Fine temperature sweep with settle-split calibration ===\n", flush=True)
for T in [1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40]:
    fold_scores = []
    for hold in range(nR):
    ...
    print(f"  T={T:.2f}: {avg:.2f}  [{fstr}]", flush=True)

# Also test with more trials at the top candidates for lower variance
print("\n=== High-confidence sweep (50 trials) for top candidates ===\n", flush=True)
