"""Test class-specific temperature and calibration clip range interactions."""
...
def apply_temperature(pred_flat, T, cls_grid_flat=None, class_temps=None):
    """Apply temperature scaling. If class_temps is provided, use per-class T."""
    if class_temps is not None and cls_grid_flat is not None:
        result = pred_flat.copy()
        for c in range(NC):
            mask = cls_grid_flat == c
            if not mask.any(): continue
            flat = pred_flat[mask]
            log_p = np.log(np.clip(flat, 1e-10, None))
            scaled = log_p / class_temps[c]
            scaled -= scaled.max(axis=-1, keepdims=True)
            exp_p = np.exp(scaled)
            prob = exp_p / exp_p.sum(axis=-1, keepdims=True)
            result[mask] = prob
        result = np.clip(result, CLIP, None)
        result /= result.sum(axis=-1, keepdims=True)
        return result
    else:
        log_p = np.log(np.clip(pred_flat, 1e-10, None))
        scaled = log_p / T
        scaled -= scaled.max(axis=-1, keepdims=True)
        exp_p = np.exp(scaled)
        prob = exp_p / exp_p.sum(axis=-1, keepdims=True)
...
for hold in range(nR):
    ...
    preds_raw = []  # raw HGB predictions before temperature
    for ig, _ in rounds[hold]:
        ...
        preds_raw.append(p.reshape(H, W, NC))
...
def run_calibration(preds_list, grids, gts, n_trials=20):
    """Run settle-split calibration and return avg score."""
...
# Test 1: Calibration clip range interaction with temperature
print("=== Clip range interaction with T=1.15 ===\n", flush=True)
for clip_lo, clip_hi in [(0.001, 10), (0.005, 50), (0.01, 100), (0.02, 200), (0.05, 500)]:
    fold_scores = []
    for hold in range(nR):
        ...
        for p_raw in c["preds_raw"]:
            p_flat = p_raw.reshape(-1, NC)
            p_scaled = apply_temperature(p_flat, 1.15)
            preds.append(p_scaled.reshape(H, W, NC))
                ...
                for ns in [True, False]:
                    for ic in range(NC):
                        ...
                        if n < 10:
                            ...
                            of=settle_obs[ns][ic]/n; pa=settle_pred[ns][ic]/n
                        r = np.where(pa > .01, np.clip(of/pa, clip_lo, clip_hi), 1.)
                        mask = (cls==ic) & (ns_flat==ns)
                        p[mask] *= r
                p = np.clip(p, CLIP, None); p /= p.sum(axis=-1, keepdims=True)
                cal.append(p.reshape(H, W, NC))
    ...
    print(f"  clip=[{clip_lo}, {clip_hi}]: {avg:.2f}  [{' '.join(f'R{i+1}={s:.2f}' for i,s in enumerate(fold_scores))}]", flush=True)

# Test 2: Per-class temperature
print("\n=== Per-class temperature (fixing others at 1.15) ===\n", flush=True)
CLASS_NAMES = ["Empty", "Settle", "Port", "Ruin", "Forest", "Mountain"]
for target_class in range(NC):
    best_T = 1.15
    best_score = 0
    for T_cls in [0.8, 0.9, 1.0, 1.1, 1.15, 1.2, 1.3, 1.5]:
        class_temps = [1.15] * NC
        class_temps[target_class] = T_cls
        fold_scores = []
        for hold in range(nR):
            c = cached[hold]; grids, gts = c["grids"], c["gts"]; H, W = c["H"], c["W"]
            preds = []
            for p_raw, g in zip(c["preds_raw"], grids):
                cls_flat = build_class_grid(g).ravel()
                p_flat = p_raw.reshape(-1, NC)
                p_scaled = apply_temperature(p_flat, 1.0, cls_flat, class_temps)
                preds.append(p_scaled.reshape(H, W, NC))
            # Quick single-trial eval (for speed)
            rng = np.random.RandomState(42+hold)
            vps = [(r,cc) for r in grid_positions(H) for cc in grid_positions(W)]
            cls_obs = np.zeros((NC,NC)); cls_pred = np.zeros((NC,NC)); cls_n = np.zeros(NC)
            settle_obs = {True: np.zeros((NC,NC)), False: np.zeros((NC,NC))}
            settle_pred = {True: np.zeros((NC,NC)), False: np.zeros((NC,NC))}
            ...
            for si in range(len(grids)):
                ...
                p = np.clip(p, CLIP, None); p /= p.sum(axis=-1, keepdims=True)
                cal.append(p.reshape(H, W, NC))
            fold_scores.append(np.mean([kl_score(gts[si], cal[si]) for si in range(len(gts))]))
        avg = np.mean(fold_scores)
        if avg > best_score:
            best_score = avg; best_T = T_cls
    print(f"  {CLASS_NAMES[target_class]:>10s}: best T={best_T:.2f} → {best_score:.2f}", flush=True)
