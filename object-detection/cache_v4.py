"""Cache v4 detections to pickle file (standalone to avoid memory issues)."""
import time, pickle
...
CACHE_FILE = Path("cache_v4_29.pkl")
...
def main():
    ...
    print(f"Caching v4 @ [1280, 1536] on {len(all_imgs)} images")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Loading {WEIGHTS_V4}...")
    model = YOLO(str(WEIGHTS_V4), task="detect")
