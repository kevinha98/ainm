...
Strategy - multi-scale + triple-model ensemble with WBF + soft-NMS:
  ...
  8 passes + 3 flip TTA passes = 11 total, WBF iou=0.50, weights tuned
  Soft-NMS: sigma=0.85 (competition-proven), iou=0.35, score=1e-5, max_dets=400
  Category voting: quadratic score-weighted (score², V9 experiment E5)

  Adaptive time budget:
    FULL:   v3@1280+1408+1536 + v4@1280+1536 + v6@1280+1408+1536 + flip(v4@1280,v6@1280+1408) -> WBF -> soft-NMS (11 passes)
...
MAX_DETS_PER_IMAGE = 400  # Competition-proven: fewer high-confidence dets > many low-confidence
...
def _soft_nms_class_agnostic(dets, iou_thresh=0.45, sigma=0.5, score_thresh=0.01):
    ...
    # Category voting (quadratic score weighting — V9 experiment E5: +0.00031 cls_mAP)
...
def _run_flip_tta(model, img_path, device, detection_only, imgsz=1280):
    """Run inference on horizontally flipped image and flip bboxes back."""
    import numpy as np
    ...
    results = model(arr, device=device, verbose=False,
                    conf=0.01, iou=0.7, max_det=600, imgsz=imgsz, augment=False)
...
def _infer_full(model_v3, model_v4, img_path, device, detection_only, model_v6=None):
    """FULL: 8 regular + 3 flip TTA = 11 passes, WBF + soft-NMS.
    v3@3scales + v4@2scales + v6@3scales + flip(v4@1280 + v6@1280+1408)."""
    from PIL import Image
    img = Image.open(img_path)
    ...
    r2 = model_v3(str(img_path), device=device, verbose=False,
                  conf=0.01, iou=0.7, max_det=600, imgsz=1408, augment=False)
    pass_v3_1408 = _extract_dets(r2, detection_only)

    r3 = model_v3(str(img_path), device=device, verbose=False,
                  conf=0.01, iou=0.7, max_det=600, imgsz=1536, augment=False)
    ...
    # Flip TTA: run v4 and v6 on horizontally flipped image (highest-impact models)
    if model_v4:
