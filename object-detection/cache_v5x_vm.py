"""Cache v5x (YOLOv8x) detections on GCP VM. Run at ~/object-detection/"""
import time, pickle, json
from pathlib import Path
from PIL import Image

YOLO_VAL_DIR = Path("data/yolo/images/val")
WEIGHTS_V5X = Path("runs/detect/train_v5x/weights/best.pt")
CACHE_FILE = Path("cache_v5x_29.pkl")
...
def main():
    ...
    print(f"Caching v5x @ [1280, 1408, 1536] on {len(all_imgs)} images")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Loading {WEIGHTS_V5X}...")
    model = YOLO(str(WEIGHTS_V5X), task="detect")
    ...
    print(f"v5x done in {time.time()-t0:.0f}s")
    with open(CACHE_FILE, "wb") as f:
        pickle.dump(cache, f)
    print(f"Saved to {CACHE_FILE} ({CACHE_FILE.stat().st_size / 1024 / 1024:.1f} MB)")
