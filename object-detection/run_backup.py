...
Strategy - multi-scale + triple-model ensemble with WBF + soft-NMS:
  ...
  Sweep v8 3-model: 8 passes, WBF iou=0.50, weights=[1,1,1,2,2,2,2,2] -> local 0.9490
...
MAX_DETS_PER_IMAGE = 300  # Cap detections per image
...
def _infer_full(model_v3, model_v4, img_path, device, detection_only, model_v6=None):
    ...
    if model_v6:
        ...
        all_weights.extend([2, 2, 2])

    # WBF fusion
    try:
        fused = wbf_fuse(all_passes, img_w, img_h, iou_thresh=0.50, skip_box_thresh=0.005,
                         weights=all_weights)
        return _soft_nms_class_agnostic(fused, iou_thresh=0.45, sigma=5.0, score_thresh=0.001)
    except Exception:
...
def _infer_medium(model_v3, model_v4, img_path, device, detection_only, model_v6=None):
    ...
    if model_v6:
        ...
        all_weights.extend([2, 2])

    try:
        fused = wbf_fuse(all_passes, img_w, img_h,
                         iou_thresh=0.50, skip_box_thresh=0.005,
                         weights=all_weights)
        return _soft_nms_class_agnostic(fused, iou_thresh=0.45, sigma=5.0, score_thresh=0.001)
    except Exception:
        all_dets = [d for p in all_passes for d in p]
        return _soft_nms_class_agnostic(
            ...
            iou_thresh=0.45, score_thresh=0.001)
...
def _infer_fast(model_v3, img_path, device, detection_only):
    ...
    return _soft_nms_class_agnostic(dets, iou_thresh=0.45, sigma=5.0, score_thresh=0.001)
