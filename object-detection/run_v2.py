...
Strategy — adaptive time-budgeted inference with WBF fusion:
  - FULL mode:   Multi-scale SAHI (640+960) + full-image pass, WBF fusion
  - MEDIUM mode: Single-scale SAHI (640) + full-image pass, WBF fusion
  - FAST mode:   Full-image pass only, no slicing

Automatically downgrades if running out of time (300s sandbox limit).
Uses Weighted Boxes Fusion (ensemble-boxes) instead of simple NMS for
better merging of overlapping detections from multi-scale SAHI tiles.
"""
import argparse
import json
import time
...
def wbf_per_image(dets, img_w, img_h, iou_thresh=0.55, skip_box_thresh=0.01):
    """Apply Weighted Boxes Fusion to merge detections from multiple passes.

    WBF averages box coordinates weighted by confidence, producing more
    accurate boxes than NMS which just picks the highest-scoring one.
    """
    ...
    # Group detections by source pass for WBF
    # WBF takes lists of [boxes_list, scores_list, labels_list] per "model"
    # We treat each detection as coming from a single "model" ensemble
    boxes = []
    scores = []
    labels = []
    for d in dets:
    ...
    # WBF expects lists of arrays per "model" — treat all as one model
    boxes_list = [np.array(boxes, dtype=np.float32)]
    scores_list = [np.array(scores, dtype=np.float32)]
...
def _infer_full(model, img_path, device, detection_only, use_tta=False):
    """FULL: full-image + multi-scale SAHI (640+960) with WBF fusion."""
    import numpy as np
    ...
    # Full-image pass (best for large/medium objects)
    ...
    # SAHI sliced passes at 640 (good for small products)
    slices_640 = _get_slices(img_w, img_h, slice_size=640, overlap=0.2)
    for cx, cy, cw, ch in slices_640:
    ...
    # SAHI sliced passes at 960 (good for medium products, better context)
    slices_960 = _get_slices(img_w, img_h, slice_size=960, overlap=0.2)
    for cx, cy, cw, ch in slices_960:
        crop_img = Image.open(img_path).crop((cx, cy, cx + cw, cy + ch))
        crop_arr = np.array(crop_img)
        results = model(crop_arr, device=device, verbose=False,
                        conf=0.05, iou=0.7, max_det=300, augment=False)
    ...
    # Use WBF to intelligently merge all detections
    try:
        return wbf_per_image(all_dets, img_w, img_h, iou_thresh=0.55)
    except Exception:
        return nms_per_image(all_dets, iou_thresh=0.5)


def _infer_medium(model, img_path, device, detection_only, use_tta=False):
    """MEDIUM: full-image + single-scale SAHI (640) with WBF fusion."""
    import numpy as np
    from PIL import Image
...
def _infer_fast(model, img_path, device, detection_only):
    """FAST: full-image pass, no slicing."""
    results = model(str(img_path), device=device, verbose=False,
    ...
    for idx, img_path in enumerate(images):
        ...
        if images_left > 0:
            ...
            if time_per_image < 1.5:
                ...
                mode = "MEDIUM"
            # FULL mode needs ~8-12s/image on L4 GPU with multi-scale SAHI
